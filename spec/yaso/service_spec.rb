RSpec.describe Yaso::Service do
  ...
  describe ".call" do
    ...
    let(:params) { {one: FFaker::Lorem.word} }
    let(:entry) { instance_double(Yaso::Steps::Base) }

    before do
      allow(Yaso::Flows::Classic).to receive(:call).and_return(entry)
    ...
    it "calls Flow to build a graph on the first call" do
      result
      expect(Yaso::Flows::Classic).to have_received(:call)
    end
    ...
    context "when service is called more than once" do
      ...
      it "calls Flow only on the first call" do
        ...
        expect(Yaso::Flows::Classic).to have_received(:call).once
      end
    end
  ...
  describe ".flow" do
    ...
    describe "when no params are passed" do
      it "returns Yaso::Flows::Classic" do
        expect(klass.flow).to eq Yaso::Flows::Classic
      end
    end
    ...
    describe "when flow name is passed" do
      ...
      it "returns specified flow class" do
        expect(klass.flow).to eq Yaso::Flows::Rollback
      end

      it "sets flow for child classes" do
        expect(child_class.flow).to eq Yaso::Flows::Rollback
      end
    end
    ...
    describe "when flow is set for child class" do
      ...
      it "does not set flow for Yaso::Service" do
        expect(described_class.flow).to eq Yaso::Flows::Classic
      end

      it "does not set flow for another services" do
        expect(another_class.flow).to eq Yaso::Flows::Classic
      end
    end
  end
end
